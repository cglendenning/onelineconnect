...
public class CUtils
{
    ...
    public static void ShowInterstitialAd()
    {
        ...
        {
            Debug.Log("Interstitial ad blocked - running in simulator");
            return;
        }
        
        // Check if ads are removed FIRST - this is the most important check
        if (IsAdsRemoved())
        {
            Debug.Log("Interstitial ad blocked - ads removed");
            return;
        }
        
        if (IsBuyItem()) return;
        // Removed the fixed frequency check since we're now using randomized frequency

        if (IsActionAvailable("show_ads", GameConfig.instance.adPeriod))
        {
#if UNITY_ANDROID || UNITY_IPHONE
            bool result = AdmobController.instance.ShowInterstitial();
            if (result == false && !SimulatorDetector.IsRunningInSimulator()) 
            {
    ...
    // Add a method to check if ads should be shown at all
    public static bool ShouldShowAds()
    {
        return !SimulatorDetector.IsRunningInSimulator() && !IsAdsRemoved() && !IsBuyItem();
    }
    ...
    public static void ShowBannerAd()
    {
        ...
        if (SimulatorDetector.IsRunningInSimulator())
        {
            Debug.Log("Banner ad blocked - running in simulator");
            return;
        }
